...
for INSTANCE in worker-0 worker-1 worker-2; do
    ...
    if ! scp "${CA_KEYS_DIR}/ca.pem" "${WORKER_KEYS_DIR}/${INSTANCE}-key.pem" "${WORKER_KEYS_DIR}/${INSTANCE}.pem" ubuntu@${WORKER_EXTERNAL_IP}:~/; then
...
for INSTANCE in controller-0 controller-1 controller-2; do
    ...
    if ! scp "${CA_KEYS_DIR}/ca.pem" "${CA_KEYS_DIR}/ca-key.pem" "${KUBE_KEYS_DIR}/kubernetes-key.pem" "${KUBE_KEYS_DIR}/kubernetes.pem" "${SERVICE_ACCOUNT_DIR}/service-account-key.pem" "${SERVICE_ACCOUNT_DIR}/service-account.pem" ubuntu@${CONTROLLER_EXTERNAL_IP}:~/; then
        echo "Failed to SCP keys to ${INSTANCE}"
        exit 1
    fi
done
